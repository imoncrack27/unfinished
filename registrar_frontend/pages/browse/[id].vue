import { Document } from '../../.nuxt/components';
<script setup>
const route = useRoute()

const { data: document } = await useFetch('http://127.0.0.1:8000/api/v1/documents/' + route.params.id + '/')

useSeoMeta({
    title: document.value.title,
    ogTitle: document.value.title,
    description: document.value.description
})

</script>
<template>
    <div class="py-10 px-6 grid md:grid-cols-4 gap-3">
        <div class="md:col-span-3">
            <h1 class="mb-6 text-3xl">{{ document.title }}</h1>

            <p>
                {{ document.description }}
            </p>

            <a v-bind:href="'mailto:' + document.company_email"
                class="inline-block mt-6 py-4 px-6 bg-teal-700 text-white rounded-xl">Status</a>
        </div>

        <div class="md:col-span-1 p-6 bg-teal-700 text-white rounded-xl">
            <h3 class="mb-6 text-2xl ">Document</h3>

            <p class="mb-2">{{ document.company_location }} </p>

            <hr class="my-6 opacity-30">

            <h3 class="mb-6 text-2xl ">Position</h3>
            <p class="mb-2">{{ document.position_status }}</p>

            <p class="mb-2">{{ document.position_location }}</p>
            <p>Payment</p>

            <hr class="my-6 opacity-30">

            <p>Posted at {{ document.created_at_formatted }}</p>
        </div>
    </div>
</template>
...
let { data: documents } = await useFetch('http://127.0.0.1:8000/api/v1/documents/', {
    ...
    <div class="grid md:grid-cols-4 gap-3 py-10 px-6">
        <div class="md:col-span-1 px-6 py-6 bg-teal-700 rounded-xl">
            <div class="flex space-x-4">
                <input v-model="query" type="search" placeholder="Find a document..." class="w-full px-6 py-4 rounded-xl">

                <button class="px-6 py-4 bg-teal-900 text-white rounded-xl" v-on:click="performSearch">
            ...
            <p v-for="category in documentCategories" v-bind:key="category.id"
                v-on:click="$event => toggleCategory(category.id)" class="py-4 px-6 text-white rounded-xl"
